...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_attention_probs=False):
        ...
        #mask_t = torch.zeros(attention_scores.shape).cuda()
        #mask_t[attention_scores.detach() < 0.3] = 1.0
        #attention_scores[attention_scores < attention_scores.mean()] = 0

        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores)
        # attention_probs = self.gate(attention_probs-0.1)
